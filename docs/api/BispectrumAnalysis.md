...
| Python import | `from qstream import BispectrumAnalysis` |

## Signature

```python
BispectrumAnalysis(window=64, nfft=64, update_every=16)
```

## What it computes

Third-order spectrum detecting quadratic phase coupling between frequency components.
...
**Output:** `PyBispectrumResult | None`.

See [PyBispectrumResult](/api/PyBispectrumResult) for its output fields.
